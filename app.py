...
def jalankan_scanner(tickers, tgl, jam):
    ...
    for i, ticker in enumerate(tickers):
        try:
            ...
            # 2. Ambil data Daily untuk harga terakhir
            ...
            # Bersihkan timezone agar bisa dicocokkan
            data_raw.index = data_raw.index.tz_localize(None)
            match = data_raw[data_raw.index.strftime('%H:%M') == jam]
            
            # Jika jam tidak pas, ambil yang paling mendekati jam tersebut
            if match.empty:
                match = data_raw.asof(datetime.combine(tgl, datetime.strptime(jam, "%H:%M").time()))
                # Jika asof mengembalikan series, ubah ke dataframe mini
            ...
            if gain_h_pct > 21.26:
                ...
                res = {
                    ...
                    "Pos": "> S1" if last_c >= s1 else "> CL" if last_c >= cl else "< CL",
                    "S1": s1, "S2": s2, "CL": cl, "Target": round_bei(target_val),
                    "sort": gain_h_pct
                }
                results.append(res)
                
        except Exception as e:
            # st.write(f"Error di {ticker}: {e}") # Aktifkan ini jika ingin debug detail
            continue
